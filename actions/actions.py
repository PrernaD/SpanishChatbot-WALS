...
class ActionCountriesLanguageSearch(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if len(entities) > 0:
            ...
            if len(out_row) > 0:
                out_row = out_row[0]
                out_text = "Estos idiomas se hablan en %s : %s\n" % (out_row["country"], out_row["language"])
                dispatcher.utter_message(text = out_text)
            else:
                dispatcher.utter_message(text = "Lo sentimos, no tenemos registros para el pais %s . :( " % query_lang)
        else:
            dispatcher.utter_message(text="Lo siento, no pude entender :(")
...
class ActionTenseSearch(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        wals_data = pd.read_csv(data_path)
        entities = list(tracker.get_latest_entity_values("tense"))
        ...
        if len(entities) > 0:
            ...
            dispatcher.utter_message(text="Did not find entity")
...
class ActionGetPoliteness(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        entities = list(tracker.get_latest_entity_values("polite"))
        ...
        if len(entities) > 0:
            ...
            if len(out_row) > 0:
                ...
                dispatcher.utter_message(text = "Nothing for  %s" % query_lang)
        else:
            dispatcher.utter_message(text="lo siento")
